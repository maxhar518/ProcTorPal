...
import { Loader2, Plus, Trash2, ArrowLeft, Save, Settings2, ListPlus } from "lucide-react";
import { toast } from "sonner";
import { getQuiz, updateQuiz, upsertQuestion, deleteQuestion } from "@/lib/quizzes/quiz.functions";

export const Route = createFileRoute("/_authenticated/quizzes/$quizId/edit")({
  head: () => ({ meta: [{ title: "Edit quiz — ProctorPal" }] }),
...
function EditQuizPage() {
  ...
  const delQM = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: quizQueryKey });
    },
  });
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <Settings2 className="h-4 w-4 text-primary" /> Quiz details
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <Input type="number" min={0} value={tl} onChange={(e) => setTl(e.target.value)} />
              </div>
              <div>
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <ListPlus className="h-5 w-5 text-primary" /> Questions
          </h2>
        ...
        <div className="space-y-4">
          ...
          {drafts.map((q, idx) => (
            <QuestionEditor
              ...
              onDelete={() => {
                if (confirm("Delete question?")) removeDraft(idx);
              }}
            />
          ))}
        </div>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Select
              value={draft.type}
              onValueChange={(v) => update("type", v as "single" | "multi")}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
            ...
            <Button
              ...
              onClick={() =>
                update("options", [
                  ...draft.options,
                  {
                    label: `Option ${String.fromCharCode(65 + draft.options.length)}`,
                    is_correct: false,
                  },
                ])
              }
            ...
            <RadioGroup value={draft.options.findIndex((o) => o.is_correct).toString()}>
              {draft.options.map((o, i) => (
                <div key={i} className="flex items-center gap-2">
                  <RadioGroupItem
                    value={i.toString()}
                    onClick={() => updateOpt(i, { is_correct: true })}
                  />
                  <Input
                    value={o.label}
              ...
              {draft.options.map((o, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Checkbox
                    ...
                    onCheckedChange={(c) => updateOpt(i, { is_correct: !!c })}
                  />
                  <Input
                    value={o.label}
                    onChange={(e) => updateOpt(i, { label: e.target.value })}
